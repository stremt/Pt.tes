  ...
  return (
    <div className="min-h-screen py-8 sm:py-12 md:py-16 lg:py-20 bg-gradient-to-b from-background via-background to-muted/20">
      <div className="container mx-auto px-2 sm:px-4 max-w-7xl">
        {/* Page Header */}
        <div className="text-center space-y-4 sm:space-y-6 md:space-y-8 mb-10 sm:mb-14 md:mb-20 px-2 sm:px-4">
          <Badge variant="secondary" className="text-sm sm:text-base px-4 sm:px-6 py-1.5 sm:py-2 font-semibold">Our Blog</Badge>
        ...
        {isLoading ? (
          ...
          <div className="space-y-10 sm:space-y-12 md:space-y-16">
            {/* Featured Post */}
            {posts[0] && (
              <Card className="overflow-hidden hover-elevate active-elevate-2 transition-all duration-300 group" data-testid={`card-blog-${posts[0].slug}`}>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                  {posts[0].featuredImage && (
                    <div className="relative h-56 sm:h-64 md:h-72 lg:h-full min-h-[250px] sm:min-h-[300px] md:min-h-[350px] lg:min-h-[450px] overflow-hidden">
                      <img 
                      ...
                      <Badge variant="default" className="absolute top-4 left-4 sm:top-6 sm:left-6 text-xs sm:text-sm px-3 py-1.5 sm:px-4 sm:py-2 font-semibold">
                        Featured Article
                      </Badge>
                    </div>
                  )}
                  <div className="p-6 sm:p-8 md:p-10 lg:p-12 flex flex-col justify-center">
                    <CardHeader className="p-0 space-y-3 sm:space-y-4 md:space-y-5">
                      <CardTitle className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold leading-tight group-hover:text-primary transition-colors duration-200">
                        {posts[0].title}
                      </CardTitle>
                    ...
                    <CardContent className="p-0 space-y-4 sm:space-y-5 md:space-y-6 mt-4 sm:mt-5 md:mt-6">
                      <CardDescription className="text-sm sm:text-base md:text-lg leading-relaxed line-clamp-3">
            ...
            {posts.length > 1 && (
              <>
                <div className="flex items-center gap-2 sm:gap-3 mb-6 sm:mb-8 px-2 sm:px-0">
                  <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold">Latest Articles</h2>
                  <Badge variant="outline" className="text-xs sm:text-sm font-semibold px-2 sm:px-3 py-0.5 sm:py-1">{posts.length - 1}</Badge>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
