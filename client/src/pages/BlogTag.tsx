...
import { Calendar, User, ArrowRight, Clock, Eye, Tag as TagIcon } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { BlogPost } from "@shared/schema";

export default function BlogTag() {
  const [, params] = useRoute("/blogs/tag/:tag");
  ...
  const tagPosts = useMemo(() => {
    if (!posts || !tagSlug) return [];
    const decodedTag = decodeURIComponent(tagSlug).toLowerCase();
    return posts.filter(post => 
      post.tags.some(tag => tag.toLowerCase() === decodedTag)
    );
  }, [posts, tagSlug]);

  const tag = tagSlug ? decodeURIComponent(tagSlug) : '';

  useSEO({
    title: `${tag} Articles | Pixocraft Blog`,
    description: `Browse all articles tagged with ${tag}. Expert guides, tutorials, and tips from the Pixocraft team.`,
    keywords: `${tag}, blog, articles, guides, tutorials, pixocraft`,
    canonicalUrl: `https://tools.pixocraft.in/blogs/tag/${tagSlug}`,
  });

  const popularPosts = useMemo(() => {
    return [...tagPosts].sort((a, b) => b.views - a.views).slice(0, 3);
  }, [tagPosts]);

  const latestPosts = useMemo(() => {
    return [...tagPosts].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [tagPosts]);
        ...
        <div className="container mx-auto px-4 max-w-3xl text-center">
          <div className="space-y-6">
            <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-full bg-muted/50 flex items-center justify-center mx-auto">
              <TagIcon className="h-10 w-10 sm:h-12 sm:w-12 text-muted-foreground" />
            </div>
      ...
      <div className="container mx-auto px-2 sm:px-4 max-w-7xl">
        ...
        <div className="text-center space-y-4 sm:space-y-6 md:space-y-8 mb-10 sm:mb-14 md:mb-16 px-2 sm:px-4">
          <div className="flex items-center justify-center gap-3">
            <TagIcon className="h-8 w-8 sm:h-10 sm:w-10 text-primary" />
            <Badge variant="secondary" className="text-sm sm:text-base px-4 sm:px-6 py-1.5 sm:py-2 font-semibold" data-testid="badge-tag">
              #{tag}
            </Badge>
          </div>
        ...
        <div className="text-center pt-12 mt-12 border-t px-2 sm:px-0">
          <Link href="/blogs">
            <Button variant="outline" size="default" className="text-sm sm:text-base" data-testid="button-back-to-all">
              Browse All Articles
