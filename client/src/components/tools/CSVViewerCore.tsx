...
export interface CSVViewerCoreHandle {
  scrollToUpload: () => void;
  loadSampleData: () => void;
}
...
const CSVViewerCore = forwardRef<CSVViewerCoreHandle, object>((_props, ref) => {
      ...
      Papa.parse(content, {
        ...
        complete: (results) => {
          if (results.data && results.data.length > 0) {
            ...
            setHistoryIndex(0);
            setHistoryLen(1);
            toast({ title: "Success", description: `Loaded ${results.data.length} rows` });
          }
        },
        error: (error: Error) => {
  ...
  useEffect(() => {
    if (data.length > 0 && !hasRestoredScrollRef.current && restoreScrollTopRef.current !== null) {
      ...
      const apply = () => { if (tableScrollRef.current) tableScrollRef.current.scrollTop = targetScroll; };
      apply();
      setTimeout(apply, 50);
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      csvStorage.saveCurrentFile(data, headers, fileName)
        .catch(() => {})
        .finally(() => { savingTimerRef.current = setTimeout(() => setIsSaving(false), 1800); });
    } else {
      csvStorage.clearCurrentFile().catch(() => {});
    }
  ...
  useEffect(() => {
    ...
    if (cell) { cell.focus(); pendingFocusRef.current = null; }
  });
  ...
  const undo = () => {
    if (historyIdxRef.current > 0) {
      ...
      if (diff) {
        setTimeout(() => { scrollToCell(diff.rowIndex, diff.colKey); triggerCellFlash(diff.rowIndex, diff.colKey); }, 80);
      }
    }
  };
  ...
  const deleteColumn = (colKey: string) => {
    ...
    const newData = data.map((row) => { const { [colKey]: _, ...rest } = row; return rest; });
    setData(newData);
    pushToHistory(newData);
  };
  ...
  const renameColumn = (oldKey: string, newKey: string) => {
    ...
    const newData = data.map((row) => { const { [oldKey]: val, ...rest } = row; return { ...rest, [newKey]: val }; });
    setData(newData);
    pushToHistory(newData);
  };
  ...
  const handleCellChange = (value: string) => setEditCellValue(value);
  ...
  const handleKeyDown = (e: React.KeyboardEvent, rowIndex: number, colIndex: number) => {
    ...
    if (e.key === "Enter" || e.key === "Tab") {
      if (editCell) {
        ...
        if (nextFilteredRow >= 0 && nextFilteredRow < filteredData.length && nextCol >= 0 && nextCol < headers.length) {
          ...
          if (nextDataIdx !== undefined) {
            ...
            setTimeout(() => { setEditCellValue(nextVal); setEditCell({ rowIndex: nextDataIdx, colKey: nextColKey }); }, 0);
          }
        }
      } else {
        e.preventDefault();
      ...
      if (e.key === "ArrowDown") nextRow = Math.min(filteredData.length - 1, rowIndex + 1);
  ...
  const filteredData = useMemo(() => {
    ...
    let result = data
      ...
      .filter(({ row }) => lc === "" || Object.values(row).some((val) => String(val).toLowerCase().includes(lc)));
    if (sortConfig) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={showSavePrompt} onOpenChange={(open) => { if (!open) skipSave(); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Save to browser history?</DialogTitle>
            <DialogDescription>
              Save <strong>{fileName || "this file"}</strong> to your browser so you can reload it quickly next time — without re-uploading.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={skipSave}>No thanks</Button>
          ...
          {showPaste && (
            <Card className="flex flex-col md:h-[400px]">
              ...
              <CardContent className="flex-1 flex flex-col gap-4">
                ...
                />
                <Button className="w-full" disabled={!pastedContent.trim()} onClick={() => handleCsvContent(pastedContent)}>
                  View Data
                </Button>
              </CardContent>
            </Card>
      ...
      {data.length > 0 && (
        <ConditionalPortal active={isFullScreen}>
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 border-b bg-muted/30">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex items-center gap-1 bg-background p-1 rounded-md border border-muted-foreground/20">
                  ...
                  <Button variant="ghost" size="sm" onClick={redo} disabled={historyIndex >= historyLen - 1} className="h-7 px-2 hover:bg-muted">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    className={cn("h-9 w-9", highlightEnabled && "text-primary bg-primary/10")}
            ...
            <div
              ref={tableScrollRef}
              className={cn("relative overflow-auto border-t", isFullScreen ? "flex-1" : "max-h-[700px]")}
                  ...
                  <TableRow className="hover:bg-transparent">
                    ...
                    {isEditing && (
                      <TableHead className="sticky top-0 z-50 w-12 p-0 border-b-2 border-muted-foreground/30 bg-muted">
                        <Button variant="ghost" size="icon" className="h-full w-full rounded-none hover:bg-primary/10 text-primary" onClick={addColumn}>
                  ...
                  {visibleRows.map(({ row, dataIdx }, localIdx) => {
                    ...
                    return (
                      <TableRow
                        ...
                        className={cn(
                          "group",
                          flashRowObj === row ? "animate-csv-flash" : "transition-colors hover:bg-primary/5",
                        )}
                      >
                        ...
                        {headers.map((header, colIndex) => (
                          <TableCell
                            ...
                            onKeyDown={(e) => handleKeyDown(e, filteredIdx, colIndex)}
                            data-row={filteredIdx}
                            data-col={colIndex}
                          >
                            {editCell?.rowIndex === dataIdx && editCell?.colKey === header ? (
                              <Input
                                autoFocus
                              ...
                              <div className="px-3 py-2 whitespace-nowrap text-sm text-foreground/90 font-medium" title={String(row[header] ?? "")}>
                                <HighlightText text={String(row[header] || "")} highlight={debouncedSearch} />
                              </div>
                            )}
                          </TableCell>
                        ))}
...
CSVViewerCore.displayName = "CSVViewerCore";

export default CSVViewerCore;
